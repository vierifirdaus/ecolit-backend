...
export const downloadPenjualanSampah = async(req:Request, res: Response) => {
    try {
        let workbook = new excelJS.Workbook();
        let worksheet = workbook.addWorksheet('Penjualan sampah');
        worksheet.columns = [
        ...
        const data = await prisma.penjualanSampah.findMany({
            orderBy:{
                date: 'asc'
            }
        });
        ...
        res.setHeader(
            'Content-Disposition', 
            'attachment; filename=' + 'penjualan_sampah.xlsx'
        );
